...
function setupAuth(){
  $("#authToggle")?.addEventListener("click",()=>{
    ...
    $("#authTitle").textContent=signup?"Create your BioStudy account":"Welcome to BioStudy";
    ...
    try{
      ...
      if(r.error){
        ...
        if(/email not confirmed/i.test(msg))
          msg="Please confirm your email first. Check your inbox, then sign in again.";
      ...
      setAuthMessage(err.message||"Something went wrong. Please try again.",true);
    }finally{
  ...
  db.auth.onAuthStateChange((event,session)=>{
    if(session && (event==="SIGNED_IN"||event==="INITIAL_SESSION") && location.pathname.endsWith("index.html"))
      go("dashboard.html");
  });
}
(async()=>{
